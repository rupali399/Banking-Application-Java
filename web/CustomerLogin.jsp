...
<html>
    ...
    <body bgcolor='pink' style="display: static;
        ...
        <%
            ...
            opdate =(String)session.getAttribute("op_date");
            
            //out.println(now);
            //out.println(dob);
            //out.println(opdate);
             
            String monthopdate = opdate.substring(5, 7);
            String  dayopdate= opdate.substring(8);
            String yearopdate= opdate.substring(0,4);
            String monthdob = dob.substring(5, 7);
            String daydob = dob.substring(8);
            String monthnow = now.substring(5, 7);
            String daynow = now.substring(8);
            String yearnow = now.substring(0, 4);
             ...
             <div><h1 class="margin:5%;">Welcome <%=session.getAttribute("login_id")%> <%=session.getAttribute("account_holder")%> Into Your Account | <%=session.getAttribute("login_time")%></h1>
               ...
               <%
                    if(daydob.equals(daynow) && monthdob.equals(monthnow))
                        out.println("<br><center><div><h1 class='margin:5%;'>Happy BirthDay</h1></div></center>");
                    if(dayopdate.equals(daynow) && monthopdate.equals(monthnow))
                    {
                        int d = Integer.parseInt(yearnow)-Integer.parseInt(yearopdate);
                        out.print("<br><center><div><h1 class='margin:5%;'>Congratulations You've completed "+d+" Years</h1></div></center>");
                    }
                %>
...
//                    formatter=new SimpleDateFormat("YYYY/MM/dd");
//           
//                    if(formatter.format(dt).equals(session.getAttribute("birth")))
//                    {
//                        out.println("<br><center><div><h1 class='margin:5%;'>Happy Birthday "+session.getAttribute("name")+"</h1></div></center>");
//                    }
